 ...
 * @return void
 */
function acf_rental_block_register_include_fields() {
    ...
    $field_json['location']   = array(
        array(
            array(
                'param'    => 'block',
                'operator' => '==',
                'value'    => 'acf/rental', // block.json name.
            ),
        ),
    );
...
add_action( 'acf/include_fields', 'acf_rental_block_register_include_fields' );
...
function acf_rental_block_categories( $block_categories ) {
	...
	$block_categories = array_merge(
		[
			[
				'slug'  => 'acf-blocks',
				'title' => __( 'ACF Blocks', 'acf_rental_block' ),
 ...
 */
function acf_rental_block_tinymce_toolbars( $toolbars ) {
    // Add a new custom TinyMCE toolbar called "Accordion"
    $toolbars['Rental']    = array();
    $toolbars['Rental'][1] = array('bold' , 'italic' , 'underline', 'link');

    return $toolbars;
}
add_filter( 'acf/fields/wysiwyg/toolbars' , 'acf_rental_block_tinymce_toolbars' );
